...
rosparam set names_map [$(for i in $(seq 0 56);do echo -ne B$i,;done|sed s/,$//)]
rosparam set names_view [$(for i in $(seq 1 57);do echo -ne A$i,;done|sed s/,$//)]
rosrun dynamic_reconfigure dynparam set /navigator "{'summaryMap': True, 'plasticMap': False,'remapRotGain': 1.0}"&
roslaunch stroll_bearnav remapTest.launch folder_map:=$f folder_view:=$f
cp ~/.ros/Results.txt results/Map_summary.txt

rosparam set names_map [$(for i in $(seq 56 88);do echo -ne B56,;done|sed s/,$//)]
