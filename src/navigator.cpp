...
void featureCallback(const stroll_bearnav::FeatureArray::ConstPtr& msg)
{
    if(state == NAVIGATING){
        ...
        if (mapDescriptors.type() != descriptorType)
        {
            ...
            mapDescriptors.release();
            mapDescriptors = Mat();
            for(size_t i = 0; i < mapFeatures.feature.size(); i++)
            {
        ...
        if (mapKeypoints.size() >0 && currentKeypoints.size() >0){
            ...
            for(size_t i = 0; i < info.view.feature.size(); i++)
            {
        ...
        if(isRating) 
        {
            ...
            if(numFeatureRemove > (int)mapFeatures.feature.size()) numFeatureAdd = numFeatureRemove = mapFeatures.feature.size();
            ...
            for(int i = 0; i < numFeatureAdd && i < (int)info.view.feature.size(); i++)
            {
        ...
        if(image_pub_.getNumSubscribers()>0)
        {
            ...
            if (currentImage.rows >0 && mapKeypoints.size() >0 && currentKeypoints.size() >0)
            {
                ...
                KeyPoint tmp;
                for(size_t i = 0; i < mapKeypoints.size(); i++)
                {
                    tmp = mapKeypoints[i];  
...
{   
    ...
    if (state == NAVIGATING){
        ...
        if(currentPathElement+2 <= (int)path.size())
        {
        ...
        {
            /*if not in demo mode*/
            if(path.size() > 0 || (showAllMatches == false && showGoodMatches == false))
            {
                state = COMPLETED;
            }
        }
        if((int)path.size()>currentPathElement)
        {
