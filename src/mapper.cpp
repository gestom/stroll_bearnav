...
vector<vector<int> > experiencesMap;
	...
	while(state == MAPPING || state == SAVING || state == PREPARING){
		...
		if(server->isPreemptRequested() || userStop)
		{
			...
			for (int i = 0;i<distanceMap.size();i++){
				...
				write(fs, "ExperienceID",experiencesMap[i]);
				fs.release();
			}
...
void featureCallback(const stroll_bearnav::FeatureArray::ConstPtr& msg)
{
	if(!isPlastic) {
		...
		if (state == SAVING) {
			...
			/* publish feedback */
			sprintf(name, "%i keypoints stored at distance %07.3f", (int) keypoints.size(), distanceTotalEvent);
			ROS_INFO("%i keypoints stored at distance %07.3f", (int) keypoints.size(), distanceTotalEvent);
			state = MAPPING;
