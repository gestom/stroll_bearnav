...
from utils.layers import conv_2d, ghh, batch_norm, get_tensor_shape, softmax
from utils.helper import get_patch_size_no_aug
from utils.dump import image_summary_nhwc
...
def process(inputs, bypass, name, skip, config, is_training):
    ...
    # ---------------------------------------------------------------------
    # Mapping layer to x,y,z
    res["score"] = softmax(res["scoremap"], axis=list(range(1, od)),
