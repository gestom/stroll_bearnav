...
for i in $(ls results/Map_adaptive_*.txt|sed s/.txt//);do  grep reports $i.txt|awk '{a=$21-$23;b=(sqrt(a*a)+384)%768-384;print sqrt(b*b)}'| tee $i.err|sort -nr > $i.srt;done

for i in $(ls results/Map_adaptive_*.txt|sed s/.txt//);do 
echo >$i.tmp
for j in $(ls results/Map_adaptive_*.txt|sed s/.txt//);do 
